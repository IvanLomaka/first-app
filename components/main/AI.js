...
// game functions
import { checkWinner, checkTie, minimaxRoot } from '../../utils/tris/trisGameHandlers'
...
export default function AI({ navigation }) {
    ...
    const onPressButton = (i) => {
        ...
        if(checkWinner(boxes, 'X')) {
            setGameFinishedMessage(`Win X`)
            setBoxes([...boxes])
        ...
        boxes[minimaxRoot(boxes)] = 'O'

        if(checkWinner(boxes, 'O')) {
            setGameFinishedMessage(`Win O`)
            setBoxes([...boxes])
            setGameFinished(true)
            return 0
        }
    ...
    return (
        <SafeAreaView style={{flex:1, backgroundColor:'#fff'}}>
            <View style={styles.header}>
                <Text style={styles.headerText}>Against AI</Text>
            </View>
